...
function filter_correct_prediction(data, comparison, accuracies)
    correct_size = 0
    incorrect_size = 0
    for row in axes(data, 1)
        if comparison[row, argmax(accuracies)] == true
            correct_size += 1
        else 
            incorrect_size += 1
        end
    ...
    incorrect_comparison = zeros(Bool, incorrect_size, 4)
    i = 1
    j = 1
    for row in axes(data, 1)
        if comparison[row, argmax(accuracies)] == true
            ...
            for col in 1:5
                ...
                incorrect_comparison[j, :] = comparison[row, :]
            end
            j += 1
        end
    end

    return correct_data, incorrect_data, incorrect_comparison
end
...
function calculate_correct_prediction_averages(data, comparison, accuracies)
    ...
    x1, remainder_data, remainder_comparison = filter_correct_prediction(data, comparison, temp_accuracies)
    ...
    x2, remainder_data, remainder_comparison = filter_correct_prediction(remainder_data, remainder_comparison, temp_accuracies)
    x2_averages = correct_prediction_sumless_mean(x2, argmax(temp_accuracies))
    averages[:, argmax(temp_accuracies)] = x2_averages
    temp_accuracies[argmax(temp_accuracies)] = 0.0

    x3, remainder_data, remainder_comparison = filter_correct_prediction(remainder_data, remainder_comparison, temp_accuracies)
    x3_averages = correct_prediction_sumless_mean(x3, argmax(temp_accuracies))
    averages[:, argmax(temp_accuracies)] = x3_averages
    temp_accuracies[argmax(temp_accuracies)] = 0.0

    x4, remainder_data, remainder_comparison = filter_correct_prediction(remainder_data, remainder_comparison, temp_accuracies)
